...
Within a contract generation, revisions MAY include documentation corrections, analyzers/test helpers/tooling/metadata improvements, non-breaking helper implementation fixes, packaging fixes, and changes that preserve all existing public requirements and conformance behavior.

Within a contract generation, revisions SHALL NOT remove required public members, change required member types/signatures, change existing semantics, make previously conformant consumers non-conformant, add new required interface members/behaviors, or silently redefine validity rules.

If a desired change cannot preserve those guarantees, a new contract generation is required.

New optional capabilities SHOULD be introduced alongside the immutable generation rather than by mutating its required surface.
...
Only then may the implementation advertise support for that generation. Release validation SHOULD fail when advertised compatibility differs from demonstrated conformance results.

The conformance suite for an immutable generation becomes part of the durable definition of what support for that generation means. Revisions to the suite SHALL NOT introduce new mandatory behavior that would invalidate a previously conformant consumer unless the earlier suite was demonstrably incorrect relative to the published contract. Such corrections require explicit review and documentation.
...
The same model applies to Engine extension contracts. Engine may advertise support for multiple contract generations only while the applicable suites pass.
...
**Conformance** answers whether an implementation can correctly support components built against a contract generation.

**Migration** answers whether an artifact, Backend, or model built for one generation can be transformed into another without changing semantics.

This ADR requires conformance testing. It does not require migration tooling.
