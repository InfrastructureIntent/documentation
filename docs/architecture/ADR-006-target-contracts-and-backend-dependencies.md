...
Target Abstractions may contain Target identity, contract-generation identity, Target model/IR types, Target expressions/references, public result/diagnostic contracts where required, compatibility metadata, and abstractions required to submit the model to the Target runtime.
...
Conformance should cover supported domain resources/features, stable diagnostics for unsupported features, managed/existing lifecycle handling, preservation of required semantic state, absence of source-recovery dependencies, and safe aggregation of independent representability failures.

## Runtime compatibility

Engine discovers Backend and Target independently and verifies Target identity, explicitly supported Target contract generation, Domain Abstractions compatibility, and early failure for incompatible combinations.
